...
<div class="relative max-w-[1440px] mx-auto px-6 py-20">
    ...
    <div class="relative bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-lg p-6 sm:p-8 max-w-lg mx-auto">
        ...
        <form method="POST" action="{{ route('study-goals.update', $studyGoal) }}" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label for="title" class="text-sm font-medium text-gray-700">Judul Goal</label>
                <input id="title" name="title" type="text" autocomplete="off" required
                       class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                       value="{{ old('title', $studyGoal->title) }}">
            </div>

            <div>
                <label for="target_hours" class="text-sm font-medium text-gray-700">Target Jam (Opsional)</label>
                <input id="target_hours" name="target_hours" type="number" min="0"
                       class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                       value="{{ old('target_hours', $studyGoal->target_hours) }}"
                       placeholder="Contoh: 10">
            </div>

            <div class="flex justify-end gap-3 pt-2">
                <a href="{{ route('study-goals.index') }}" class="flex justify-center py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    Batal
                </a>
                <button type="submit"
