...
<style>
    ...
    .form-control, .form-select { border-radius: 8px; padding: 10px 12px; border: 1px solid #ced4da; transition: all 0.2s ease; }
    .form-control:focus, .form-select:focus { border-color: #1e90ff; box-shadow: 0 0 0 0.2rem rgba(30,144,255,0.2); }
    .btn-primary { background: #1e90ff; border: none; border-radius: 10px; padding: 10px 22px; font-weight: 600; transition: 0.2s; }
...
<div class="container">
    <div class="form-card">
        <h2 class="title">✏️ Edit Study Session</h2>
        ...
        <form action="{{ route('study-sessions.update', $studySession->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="duration_minutes" id="duration_minutes" class="form-control" required min="1" value="{{ $studySession->duration_minutes }}">
            </div>

            <div class="mb-3">
                <label for="note" class="form-label">Catatan</label>
                <textarea name="note" id="note" rows="3" class="form-control">{{ $studySession->note }}</textarea>
            </div>

            <div class="d-flex justify-content-between align-items-center mt-4">
                <a href="{{ route('study-sessions.index') }}" class="btn btn-secondary px-4">⬅ Kembali</a>
